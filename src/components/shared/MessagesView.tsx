...
import {
  ...
  CornerUpLeft, AlertCircle, RotateCcw,
} from "lucide-react";
...
type GroupedItem = { type: "divider"; label: string } | { type: "message"; msg: LocalMessage };
...
function groupMessages(messages: LocalMessage[]): GroupedItem[] {
  ...
  for (const msg of messages) {
    ...
    if (d !== lastDate) { grouped.push({ type: "divider", label: formatDateDivider(msg.created_at) }); lastDate = d; }
...
export function MessagesView({ currentProfile, pairsWithPartners, initialMessages }: Props) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `pair_id=eq.${selectedPairId}` }, (payload) => {
        ...
        if (seenMessageIds.current.has(newMsg.id)) return; // already added optimistically
  ...
  const handleSend = () => {
    ...
    setInput("");
    if (replyingTo) {
      content = encodeReply(replyingTo, content);
      setReplyingTo(null);
    }
  ...
  const startPress = (msg: LocalMessage) => {
    ...
    longPressTimer.current = setTimeout(() => {
      ...
      const decoded = !sharedItem ? decodeReply(msg.content) : null;
      const snippet = decoded ? decoded.text : msg.content;
      const partnerProfile = pairsWithPartners.find((p) => p.pair.id === selectedPairId)?.partnerProfile ?? null;
      setReplyingTo({
        id: msg.id,
        senderName: msg.sender_id === currentProfile.id ? (currentProfile.display_name || "You") : getPartnerName(partnerProfile),
  ...
  if (pairsWithPartners.length === 0) {
    return (
      <div className="flex flex-col gap-6 max-w-2xl">
        <div>
          <h1 className="text-4xl font-headline font-bold tracking-tighter leading-[0.9] mb-2">SECURE<br /><span className="text-pink italic">TRANSMISSIONS</span></h1>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6 max-w-4xl">
            ...
            {pairsWithPartners.map(({ pair, partnerProfile: pp }) => {
              ...
              return (
                <button key={pair.id} onClick={() => handleSelectPair(pair.id)} className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl flex-shrink-0 border transition-all duration-200 ${isActive ? "bg-primary/10 border-primary/30 text-foreground" : "bg-surface-low border-outline-variant/10 text-muted hover:text-foreground hover:border-white/10"}`}>
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-headline font-bold border flex-shrink-0 ${isActive ? "bg-primary/20 border-primary/30 text-primary" : "bg-surface-container border-outline-variant/10 text-muted"}`}>{getInitial(pp)}</div>
        ...
        {/* Chat window */}
        <div className="flex rounded-2xl border border-outline-variant/10 overflow-hidden bg-surface-low" style={{ height: "calc(100vh - 14rem)", minHeight: "400px", boxShadow: "inset 0 0 60px rgba(168,85,247,0.03)" }}>
          {/* Main chat */}
          <div className="flex flex-col flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 px-5 py-4 bg-surface-container border-b border-white/5 flex-shrink-0">
              <div className="relative">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-headline font-bold text-sm border flex-shrink-0 ${isMistress ? "bg-pink/10 border-pink/20 text-pink" : "bg-primary/10 border-primary/20 text-primary"}`}>{getInitial(partnerProfile)}</div>
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <button onClick={() => setShowSidebar((v) => !v)} className={`p-2 rounded-lg transition-colors ${showSidebar ? "bg-primary/20 text-primary" : "text-muted hover:text-foreground hover:bg-white/5"}`} title="Share items">
                  <Paperclip size={15} />
                </button>
              </div>
            </div>
            ...
            {/* Messages */}
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-1 select-none">
              {loadingMessages ? (
                      ...
                      <div className={`max-w-[75%] sm:max-w-[65%] flex flex-col ${isOwn ? "items-end" : "items-start"}`}>
                        ...
                        >
                          {sharedItem ? (
                            ...
                            <div className={`px-4 py-2.5 rounded-2xl transition-opacity ${
                              msg._pending ? "opacity-60" : msg._failed ? "opacity-50" : "opacity-100"
                            } ${isOwn ? "bg-primary/20 border border-primary/30 rounded-br-sm" : "bg-surface-container border border-outline-variant/20 rounded-bl-sm"}`}>
                              {/* Reply quote */}
                              {replyDecoded && (
                        ...
                        {/* Timestamp + pending/failed + like indicator */}
                        <div className={`flex items-center gap-2 px-1 mt-1 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
                          ...
                          {isLiked && !msg._pending && !msg._failed && (
            ...
            <div className="flex-shrink-0 border-t border-white/5 bg-surface-container">
              ...
              <div className="px-4 py-3">
                <div className="relative flex items-end gap-2">
                  {showEmojiPicker && <EmojiPicker onSelect={(e) => { setInput((v) => v + e); inputRef.current?.focus(); }} onClose={() => setShowEmojiPicker(false)} />}
                  <button onClick={() => setShowEmojiPicker((v) => !v)} className={`flex-shrink-0 p-2.5 rounded-xl transition-colors ${showEmojiPicker ? "text-primary bg-primary/10" : "text-muted hover:text-foreground hover:bg-white/5"}`}>
                    <Smile size={18} />
                  </button>
                  <input
                    ref={inputRef}
                  ...
                  />
                  <button onClick={handleSend} disabled={!input.trim() || !selectedPairId} className="btn-gradient w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 disabled:opacity-40 transition-opacity">
                    <Send size={16} />
                  </button>
                </div>
              </div>
